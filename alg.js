...
async function compress_text_into_zip_base64(str, callback, mode="gzip") {
    try {
        ...
        writer.close();
        return await new Response(cs.readable).blob().then(blobToBase64).then(res => callback(res));
    } catch (err) {
        console.log(err);
        callback("");
    }
...
async function decompress_base64_zip_into_text(str, callback, mode="gzip") {
    try {
        ...
        writer.write(bytes)
        writer.close()
        return await new Response(cs.readable).arrayBuffer().then(arr => callback(new TextDecoder().decode(arr)), async _ => {
...
}
{
    ...
    doInclude([include("shared/point.js"), include("entities/edge/api/index.js"), include("entities/vertex/api/index.js"), include("model/Graph.js", modulDir)])
}
function Point(x, y) {
    this.x = x || 0;
    this.y = y || 0;
...
Point.hitTest = function(pt11, pt12, pt21, pt22) {
    ...
    var epsilon = 1E-5;
    if (!(res.x >= Math.min(x1, x2) - epsilon && res.x >= Math.min(x3, x4) - epsilon && res.x <= Math.max(x1, x2) + epsilon && res.x <= Math.max(x3, x4) + epsilon && res.y >= Math.min(y1, y2) - epsilon && res.y >= Math.min(y3, y4) - epsilon && res.y <= Math.max(y1, y2) + epsilon && res.y <= Math.max(y3, y4) + epsilon)) {
        res = null;
    }
    return res;
}
...
Rect.prototype.isIn = function(v) {
    return this.minPoint.x <= v.x && this.minPoint.y <= v.y && this.maxPoint.x > v.x && this.maxPoint.y > v.y;
}
;
{
    let modulDir = "entities/edge/";
    doInclude([include("model/BaseEdge.js", modulDir), include("model/EdgeModel.js", modulDir)])
}
...
BaseEdge.prototype.SaveToXML = function() {
    return "<edge " + "source=\"" + this.vertex1.id + "\" " + "target=\"" + this.vertex2.id + "\" " + "directed=\"" + this.isDirect + "\" " + "weight=\"" + this.weight + "\" " + "useWeight=\"" + this.useWeight + "\" " + "id=\"" + this.id + "\" " + "text=\"" + gEncodeToHTML(this.text) + "\" " + "upText=\"" + gEncodeToHTML(this.upText) + "\" " + "arrayStyleStart=\"" + this.arrayStyleStart + "\" " + "arrayStyleFinish=\"" + this.arrayStyleFinish + "\" " + ((Object.keys(this.ownStyles).length > 0) ? "ownStyles = \"" + gEncodeToHTML(JSON.stringify(this.ownStyles)) + "\" " : "") + this.model.SaveToXML() + "></edge>";
}
...
BaseEdge.prototype.SetWeight = function(weight) {
    ...
    weight = (!isNaN(parseInt(weight, 10))) ? weight : 1;
...
EdgeModel.prototype.HitTestLine = function(position1, position2, mousePos, factor, ignoreSelfLoop) {
    ...
    if (r1 >= (new Point(r2,r12)).length() || r2 >= (new Point(r1,r12)).length()) {} else {
...
}
{
    ...
    doInclude([include("model/BaseVertex.js", modulDir), include("model/VertexModel.js", modulDir)])
}
...
BaseVertex.prototype.SaveToXML = function() {
    return "<node " + "positionX=\"" + this.position.x + "\" " + "positionY=\"" + this.position.y + "\" " + "id=\"" + this.id + "\" " + "mainText=\"" + gEncodeToHTML(this.mainText) + "\" " + "upText=\"" + gEncodeToHTML(this.upText) + "\" " + ((Object.keys(this.ownStyles).length > 0) ? "ownStyles = \"" + gEncodeToHTML(JSON.stringify(this.ownStyles)) + "\" " : "") + "size=\"" + this.model.diameter + "\" " + "></node>";
}
...
BaseVertex.prototype.HitTest = function(pos) {
    ...
    if (shape == VertexCircleShape) {
        ...
        var pointsVertex1 = GetPointsForShape(shape, this.model.diameter + width, style != null ? {
            style: style,
            text: this.mainText
        } : null);
        pointsVertex1.push(pointsVertex1[0]);
...
BaseVertex.prototype.getBBox = function(style) {
    ...
    return new Point(Math.max(factor.x * defaultDiameter, vertexDiameter, textWidth),Math.max(factor.y * defaultDiameter, vertexDiameter + (isTextAbove ? textHeight : 0), textHeight));
}
...
Graph.prototype.FindEdgeMin = function(id1, id2) {
    ...
    for (var i = 0; i < this.edges.length; i++) {
        ...
        if ((edge.vertex1.id == id1 && edge.vertex2.id == id2) || (!edge.isDirect && edge.vertex1.id == id2 && edge.vertex2.id == id1)) {
...
Graph.prototype.FindEdgeMinIgnoreDirection = function(id1, id2) {
    ...
    for (var i = 0; i < this.edges.length; i++) {
        var edge = this.edges[i];
        if ((edge.vertex1.id == id1 && edge.vertex2.id == id2) || (edge.vertex1.id == id2 && edge.vertex2.id == id1)) {
            if (edge.weight < minWeight) {
                res = edge;
                minWeight = edge.weight;
            }
...
Graph.prototype.TestAdjacencyMatrix = function(matrix, rowsObj, colsObj, separator) {
    ...
    for (var i = 0; i < rowsObj.rows.length; i++) {
        colsObj.cols[i] = this.SplitMatrixString(rowsObj.rows[i], separator);
...
Graph.prototype.TestPair = function(pair) {
    ...
    let regExp = [/^.+-.+$/g, /^.+\>.+$/g, /^.+<.+$/g, /^.+-\(\d+\.?\d+\)-.+$/g, /^.+-\(\d+\.?\d+\)\>.+$/g, /^.+<\(\d+\.?\d+\)\-.+$/g];
    let res = true;
    ...
    while (k < 1000 && bChanged) {
        ...
        for (i = 0; i < newVertices.length; i++) {
            ...
            for (j = 0; j < this.vertices.length; j++) {
                ...
                if (otherVertex == currentVertex.object)
                    continue;
...
Graph.prototype.SetPair = function(pairs, viewportSize, currentEnumVerticesType) {
    if (this.TestPair(pairs)) {
        ...
        let regExp = [/^(.+)-(.+)$/g, /^(.+)\>(.+)$/g, /^(.+)<(.+)$/g, /^(.+)-\((-?\d+|-?\d+\.?\d+)\)-(.+)$/g, /^(.+)-\((-?\d+|-?\d+\.?\d+)\)\>(.+)$/g, /^(.+)<\((-?\d+|-?\d+\.?\d+)\)\-(.+)$/g, ];
    ...
    if (this.TestIncidenceMatrix(matrix, rowsObj, colsObj)) {
        ...
        for (var i = 0; i < cols[0].length; i++) {
            ...
            if (edgeIndex.length == 2) {
                ...
                var nEdgeIndex = this.AddNewEdgeSafe(this.vertices[edgeIndex[0]], this.vertices[edgeIndex[1]], edgeValue[0] != edgeValue[1], Math.abs(edgeValue[1]), false);
                this.FixEdgeCurve(nEdgeIndex);
                if (nEdgeIndex >= 0) {
                    bWeightGraph = bWeightGraph || this.edges[nEdgeIndex].weight != 1;
                }
...
Graph.prototype.getNeighborEdges = function(edge) {
    ...
    for (var i = 0; i < this.edges.length; i++) {
        ...
        if (curEdge == edge)
            continue;
        if ((curEdge.vertex1.id == edge.vertex1.id && curEdge.vertex2.id == edge.vertex2.id) || (curEdge.vertex1.id == edge.vertex2.id && curEdge.vertex2.id == edge.vertex1.id)) {
            res.push(curEdge);
        }
    }
    return res;
...
Graph.prototype.checkMutiGraph = function() {
    ...
    for (var i = 0; i < this.edges.length; i++) {
        var edge = this.edges[i];
        if (start.hasOwnProperty(edge.vertex1.id) && start[edge.vertex1.id] == edge.vertex2.id) {
            res = true;
            break;
        }
...
}
{
    ...
    doInclude([include("model/BaseBackgroundDrawer.js", modulDir), include("model/EdgeStyle.js", modulDir), include("model/BaseEdgeDrawer.js", modulDir), include("model/VertexShape.js", modulDir), include("model/VertexStyle.js", modulDir), include("model/BaseVertexDrawer.js", modulDir), include("model/GraphFullStyle.js", modulDir)])
}
...
CommonBackgroundStyle.prototype.saveToJson = function(field) {
    return JSON.stringify({
        commonColor: this.commonColor,
        commonOpacity: this.commonOpacity,
...
var DefaultSelectedEdgeStyles = [new SelectedEdgeStyle0(), new SelectedEdgeStyle1(), new SelectedEdgeStyle2(), new SelectedEdgeStyle3(), new SelectedEdgeStyle4()];
...
BaseEdgeDrawer.prototype.DrawArc = function(position1, position2, arcStyle) {
    ...
    if (position1.equals(position2)) {
        ...
        this.context.arc(position1.x - Math.cos(this.model.GetLoopShiftAngel()) * this.model.GetLoopSize(), position1.y - Math.sin(this.model.GetLoopShiftAngel()) * this.model.GetLoopSize(), this.model.GetLoopSize(), 0, 2 * Math.PI);
        this.context.stroke();
    } else {
        this.context.beginPath();
...
BaseEdgeDrawer.prototype.DrawWeight = function(position1, position2, text, arcStyle, hasPair) {
    ...
    this.context.beginPath();
    this.context.rect(centerPoint.x - widthText / 2 - arcStyle.textPadding / 2, centerPoint.y - arcStyle.mainTextFontSize / 1.7 - arcStyle.textPadding / 2, widthText + arcStyle.textPadding, arcStyle.mainTextFontSize + arcStyle.textPadding);
...
BaseEdgeDrawer.prototype.DrawUpText = function(position1, position2, text, arcStyle, hasPair, color, offset, fontSize) {
    var centerPoint = this.GetTextCenterPoint(position1, position2, hasPair, arcStyle);
    this.context.font = fontSize == null ? "bold " + (DefaultMainTextFontSizeEdge + TopTextFontSizeDeltaEdge) + "px sans-serif" : "bold " + (fontSize + TopTextFontSizeDeltaEdge) + "px sans-serif";
    this.context.textBaseline = "middle";
    var widthText = this.context.measureText(text).width;
...
function GetTextboxPoints(diameter, additional_data) {
    ...
    if (additional_data) {
        ...
        tempContext.font = "bold " + additional_data.style.mainTextFontSize + DefaultFont;
...
function GetPointsForShape(shape, diameter, additional_data=null) {
    ...
    case VertexSquareShape:
        return GetSquarePoints(diameter);
        break;
    case VertexTriangleShape:
        return GetTrianglePoints(diameter);
        break;
    case VertexPentagonShape:
        return GetPentagonPoints(diameter);
        break;
    case VertexTextboxShape:
        return GetTextboxPoints(diameter, additional_data);
        break;
    case VertexSnowflakeShape:
        return GetShowflakePoints(diameter);
        break;
    default:
        return null;
        break;
...
var DefaultSelectedGraphStyles = [new SelectedVertexStyle0(), new SelectedVertexStyle1(), new SelectedVertexStyle2(), new SelectedVertexStyle3(), new SelectedVertexStyle4()];
...
BaseVertexDrawer.prototype.Draw = function(baseGraph, graphStyle) {
    ...
    if (graphStyle.commonTextPosition == CommonTextCenter) {
        this.DrawCenterText(baseGraph.position, baseGraph.mainText, graphStyle.mainTextColor, graphStyle.fillStyle, true, true, graphStyle.mainTextFontSize);
        this.DrawCenterText(baseGraph.position.add(new Point(0,-shapeSize / 2.0 - graphStyle.mainTextFontSize / 2.2)), baseGraph.upText, graphStyle.upTextColor, graphStyle.strokeStyle, false, false, graphStyle.mainTextFontSize + TopTextFontSizeDelta);
    } else if (graphStyle.commonTextPosition == CommonTextUp) {
        this.DrawCenterText(baseGraph.position.add(new Point(0,-shapeSize / 2.0 - graphStyle.mainTextFontSize / 2.2)), baseGraph.mainText, graphStyle.mainTextColor, graphStyle.fillStyle, true, false, graphStyle.mainTextFontSize);
        this.DrawCenterText(baseGraph.position.add(new Point(0,shapeSize / 2.0 + graphStyle.mainTextFontSize / 1.7)), baseGraph.upText, graphStyle.upTextColor, graphStyle.strokeStyle, false, false, graphStyle.mainTextFontSize + TopTextFontSizeDelta);
    }
}
...
}
{
    ...
    doInclude([include("model/Algorithms.js", modulDir), include("model/BaseTraversal.js", modulDir)])
    function loadAsyncAlgorithms(onFinish) {
        let modulDir = "features/algorithms/";
        let pluginsList = ["BFS.js", "Coloring.js", "ConnectedComponent.js", "DFS.js", "EulerianLoop.js", "EulerianPath.js", "FindAllPatches.js", "FindLongestPath.js", "FindShortPatchsFromOne.js", "Floid.js", "GraphReorder.js", "HamiltonianLoop.js", "HamiltonianPath.js", "IsomorphismCheck.js", "MaxClique.js", "MaxFlow.js", "MinimumSpanningTree.js", "ModernGraphStyle.js", "RadiusAndDiameter.js", "ShortestPath.js", "VerticesDegree.js"];
...
BaseAlgorithmEx.prototype.CalculateAlgorithm = function(algorithmName, otherParams, resultCallback, ignoreSeparateNodes=false) {
    ...
    if (this.app.isSupportEmscripten()) {
        ...
        var processData = algorithmName + delimiter + xml + delimiter + "report" + delimiter + "xml";
...
BaseHandler.prototype.GetSelectVertexMenu = function(menuName) {
    var res = "<input list=\"vertexList" + menuName + "\" id=\"" + menuName + "\" class=\"SelectVertexInput\"/>" + "<datalist id=\"vertexList" + menuName + "\">";
    for (var i = 0; i < this.app.graph.vertices.length; i++) {
...
BaseHandler.prototype.GetSelect2VertexMenu = function() {
    return "<span style=\"float:right\">" + this.GetSelectVertexMenu("Vertex1") + " &rarr; " + this.GetSelectVertexMenu("Vertex2") + "</span>";
}
    ...
    $("#Context_Connect").click(function() {
        if (handler.contextMenuObject != null && handler.GetSelectedVertex() != null) {
            var addFunc = function(firstVertex, secondVertex, direct) {
                handler.app.CreateNewArc(firstVertex, secondVertex, direct, document.getElementById('EdgeWeight').value, $("#RadiosReplaceEdge").prop("checked"), document.getElementById('EdgeLable').value);
                handler.app.redrawGraph();
            }
...
DefaultHandler.prototype.MouseUp = function(pos) {
    ...
    if (this.selectedObject != null && (this.selectedObject instanceof BaseVertex)) {
        this.message = g_textsSelectAndMove + "<div style=\"float:right;position: relative;\">" + "<button type=\"button\" class=\"btn btn-outline-secondary dropdown-toggle btn-sm menu-text white-btn\" data-bs-toggle=\"dropdown\" aria-expanded=\"false\">" + " " + g_action + " <span class=\"caret\"></span>" + "</button>" + "<ul class=\"dropdown-menu\" style=\"z-index:15; position: absolute;\">" + " <li><a href=\"#\" class=\"dropdown-item\" id=\"renameButton\">" + g_renameVertex + "</a></li>" + " <li><a href=\"#\" class=\"dropdown-item\" id=\"changeCommonStyle\">" + g_commonVertexStyle + "</a></li>" + " <li><a href=\"#\" class=\"dropdown-item\" id=\"changeSelectedStyle\">" + g_selectedVertexStyle + "</a></li>" + "</ul>" + "</div>";
        var handler = this;
        var callback = function(enumType) {
        ...
        this.message = g_textsSelectAndMove + "<span style=\"float:right;\"><button type=\"button\" id=\"incCurvel\" class=\"btn btn-outline-secondary btn-sm menu-text white-btn\"> + </button>" + " " + g_curveEdge + " " + "<button type=\"button\" id=\"decCurvel\" class=\"btn btn-outline-secondary btn-sm menu-text white-btn\"> - </button> &nbsp; " + "<div style=\"float:right;position: relative;\">" + "<button type=\"button\" class=\"btn btn-outline-secondary dropdown-toggle btn-sm menu-text white-btn\" data-bs-toggle=\"dropdown\" aria-expanded=\"false\">" + " " + g_action + " <span class=\"caret\"></span>" + " </button>" + "<ul class=\"dropdown-menu dropdown-menu-right\" style=\"z-index:15; position: absolute;\">" + " <li><a class=\"dropdown-item\" href=\"javascript:;\" id=\"editEdge\">" + g_editWeight + "</a></li>" + " <li><a class=\"dropdown-item\" href=\"javascript:;\" id=\"changeCommonStyle\">" + g_commonEdgeStyle + "</a></li>" + " <li><a class=\"dropdown-item\" href=\"javascript:;\" id=\"changeSelectedStyle\">" + g_selectedEdgeStyle + "</a></li>" + "</ul>" + "</div>";
        var handler = this;
        $('#message').unbind();
        ...
        this.message = this.message + "<button type=\"button\" id=\"DublicateSelected\" class=\"btn btn-outline-secondary btn-sm menu-text white-btn\">" + g_copyGroupeButton + "</button> &nbsp &nbsp" + "<button type=\"button\" id=\"RemoveSelected\" class=\"btn btn-outline-secondary btn-sm menu-text white-btn\">" + g_removeGroupeButton + "</button>"
        this.message = this.message + " &nbsp &nbsp <button type=\"button\" class=\"btn btn-outline-secondary dropdown-toggle btn-sm menu-text white-btn\" data-bs-toggle=\"dropdown\" aria-expanded=\"false\">" + " " + g_action + " <span class=\"caret\"></span>" + " </button>" + "<ul class=\"dropdown-menu dropdown-menu-right\" style=\"z-index:15; position: absolute;\">";
        if (hasEdges) {
        ...
        this.message = this.message + "</ul>" + "</span>";
        var handler = this;
        $('#message').unbind();
...
ConnectionGraphHandler.prototype.AddNewEdge = function(selectedObject, isDirect) {
    let newEdge = this.app.CreateNewArc(this.firstObject, selectedObject, isDirect, document.getElementById('EdgeWeight').value, $("#RadiosReplaceEdge").prop("checked"), document.getElementById('EdgeLable').value);
...
ConnectionGraphHandler.prototype.AddDefaultEdge = function(selectedObject) {
    ...
    let newEdge = this.app.CreateNewArc(this.firstObject, selectedObject, defaultEdge.isDirect, defaultEdge.weight, false, defaultEdge.upText);
...
ConnectionGraphHandler.prototype.SelectFirst = function() {
    ...
    this.message = g_selectFirstVertexToConnect + this.GetUseDefaultEdgeCheckBox() + this.GetSelect2VertexMenu();
...
ConnectionGraphHandler.prototype.SelectSecond = function(selectedObject) {
    ...
    this.message = g_selectSecondVertexToConnect + this.GetUseDefaultEdgeCheckBox() + this.GetSelect2VertexMenu();
    this.message = this.AppendSpecialSctionsButton(this.message);
}
...
ConnectionGraphHandler.prototype.AppendSpecialSctionsButton = function(baseMessage) {
    ...
    return "<div class=\"dropdown\" style=\"float:right; position: relative; margin-left: 8px\">" + "<button type=\"button\" class=\"btn btn-outline-secondary dropdown-toggle btn-sm menu-text white-btn\" id=\"dropdownMenuForEdges\" data-bs-toggle=\"dropdown\" saria-expanded=\"false\">" + g_additionalActions + " <span class=\"caret\"></span>" + " </button> " + "<ul class=\"dropdown-menu dropdown-menu-right\" style=\"z-index:15; position: absolute;\" aria-labelledby=\"dropdownMenuForEdges\">" + (hasDirectedEdges ? " <li><a href=\"javascript:;\" class=\"dropdown-item\" id=\"reverseAll\">" + g_reverseAllEdges + "</a></li>" : "") + (hasDirectedEdges ? " <li><a href=\"javascript:;\" class=\"dropdown-item\" id=\"makeAllUndirected\">" + g_makeAllUndirected + "</a></li>" : "") + (hasUndirectedEdges ? " <li><a href=\"javascript:;\" class=\"dropdown-item\" id=\"makeAllDirected\">" + g_makeAllDirected + "</a></li>" : "") + "</ul>" + "</div> " + baseMessage;
}
...
SelectGraphDialog.prototype.show = function() {
    ...
    this.originalGraphPreview = new GraphPreview(this.originalGraph,this.originalGraphStyle,document.getElementById("OriginalGraphpPreview"),originalGraphPositionUpdate);
    this.autoSavedGraphPreview = new GraphPreview(this.autoSavedGraph,this.autoSavedGraphStyle,document.getElementById("AutoSaveGraphpPreview"),autoSavedGraphPreviewUpdate);
}
...
GraphPreview.prototype.redrawGraph = function(context, backgroundPosition) {
    ...
    backgroundDrawer.Draw(this.style.backgroundCommonStyle, Math.max(this.canvas.width, this.getRealWidth()), Math.max(this.canvas.height, this.getRealHeight()), backgroundPosition, this.canvasScale);
...
GraphPreview.prototype.getMousePos = function(canvas, e) {
    ...
    return new Point((e.clientX - rect.left) / this.canvasScale - this.canvasPosition.x,(e.clientY - rect.top) / this.canvasScale - this.canvasPosition.y);
}
...
}
{
    ...
    doInclude([include("model/GraphMLCreator.js", modulDir)])
}
...
GraphMLCreator.prototype.GetXMLString = function() {
    ...
    var weightKeyId = "\"d0\"";
    var weightNode = "<key id=" + weightKeyId + " for=\"node\" attr.name=\"weight\" attr.type=\"double\">" + "<default>" + defaultWeight + "</default>" + "</key>";
    var xmlBody = "";
    ...
    for (var i = 0; i < this.arcs.length; i++) {
        ...
        xmlBody = xmlBody + "<edge source=\"" + arc.vertex1.id + "\" target=\"" + arc.vertex2.id + "\" " + (arc.isDirect != hasDirected ? (hasDirected ? "directed=\"false\"" : "directed=\"true\"") : "") + " id=\"" + arc.id + "\"";
...
function Application(document, window, listener) {
    ...
    this.savedGraphName = "";
    this.currentEnumVerticesType = new BaseEnumVertices(this,1);
...
Application.prototype.maxAutosaveSizeForCookie = 2000;
Application.prototype.autosaveTimeInterval = 1000 * 60;
...
Application.prototype.getMousePos = function(canvas, e) {
    var rect = canvas.getBoundingClientRect();
    return {
        x: (e.clientX - rect.left) / this.canvasScale - this.canvasPosition.x,
        y: (e.clientY - rect.top) / this.canvasScale - this.canvasPosition.y
    };
}
...
Application.prototype._PrintRedrawGraph = function() {
    ...
    this._RedrawGraph(context, {
        ...
        height: canvas.height,
        scale: 1.0
    }, bbox.minPoint.inverse(), this.backgroundPrintStyle, false, this.vertexPrintCommonStyle, this.vertexPrintSelectedVertexStyles, this.edgePrintCommonStyle, this.edgePrintSelectedStyles);
    context.restore();
    return canvas;
}
...
Application.prototype.startRenderTimer = function() {
    ...
    this.renderTimer = window.setInterval(function() {
        globalApplication.redrawGraphTimer();
    }, 50);
...
Application.prototype.RedrawSelectionRect = function(context) {
    ...
    context.beginPath();
    context.rect(this.selectionRect.left(), this.selectionRect.top(), this.selectionRect.size().x, this.selectionRect.size().y);
    context.closePath();
    context.stroke();
    context.setLineDash([]);
}
...
Application.prototype.multCanvasScale = function(factor, zoom_to=null) {
    if (zoom_to) {
        var pos1 = this.getMousePos(this.canvas, zoom_to);
        this.canvasScale *= factor;
        var pos2 = this.getMousePos(this.canvas, zoom_to);
...
Application.prototype.CreateNewGraph = function(x, y) {
    var app = this;
    this.currentEnumVerticesType.GetVertexTextAsync(function(enumType) {
...
Application.prototype.SaveGraphOnDisk = function() {
    ...
    DiskSaveLoad.SaveGraphOnDisk(this.savedGraphName, graphAsString, function(msg) {
        ...
        app.removeAutosave();
        app.lastGraphName = app.savedGraphName;
...
Application.prototype.SaveGraphImageOnDisk = function(showDialogCallback) {
    ...
    if (this.IsGraphFitOnViewport()) {
        ...
        rectParams = "&x=" + Math.round(pos.x * this.canvasScale) + "&y=" + Math.round(pos.y * this.canvasScale) + "&width=" + Math.round(bbox.size().x * this.canvasScale) + "&height=" + Math.round(bbox.size().y * this.canvasScale);
    }
...
Application.prototype.LoadGraphFromDisk = function(graphName) {
    ...
    DiskSaveLoad.LoadGraphFromDisk(graphName, function(msg) {
        ...
        app.removeAutosave();
        app.lastGraphName = graphName;
    });
}
Application.prototype.GetNewGraphName = function() {
    var name = this.GetNewName();
    if (this.style.isVertexCommonStyleCustom || this.style.isVertexSelectedVertexStylesCustom || this.style.isBackgroundCommonStyleCustom || this.style.isEdgeCommonStyleCustom || this.style.isEdgeSelectedStylesCustom) {
...
Application.prototype.IsGraphFitOnViewport = function() {
    ...
    var canvasHeight = this.GetRealHeight();
    var canvasPositionInverse = this.canvasPosition.inverse();
    return (Math.floor(canvasPositionInverse.x) <= Math.floor(graphBBox.minPoint.x) && Math.floor(canvasPositionInverse.y) <= Math.floor(graphBBox.minPoint.y) && Math.floor(canvasPositionInverse.x + canvasWidth) >= Math.floor(graphBBox.maxPoint.x) && Math.floor(canvasPositionInverse.y + canvasHeight) >= Math.floor(graphBBox.maxPoint.y));
}
...
Application.prototype._RedrawGraph = function(context, canvasParams, backgroundPosition, backgroundStyle, bDrawSelectedRect, forceVertexCommon, forceVertexSelected, forceEdgeCommon, forceEdgeSelected) {
    var backgroundDrawer = new BaseBackgroundDrawer(context);
    backgroundDrawer.Draw(backgroundStyle, Math.max(canvasParams.width, this.GetRealWidth()), Math.max(canvasParams.height, this.GetRealHeight()), backgroundPosition, canvasParams.scale);
...
Application.prototype.showSelectGraphDialog = function(graphName) {
    ...
    DiskSaveLoad.LoadGraphFromDisk(graphName, function(graphOriginXML) {
        app.getAutoSaveGraph(function(xmlGraph) {
            ...
            (new SelectGraphDialog(app,originalGraph,styleOriginal,autosaveGraph,styleAutoSave,function() {
                app.onSelectOgirinalGraph(graphName);
            }
            ,function() {
...
Application.prototype.UpdateLastUsedGraphsMenu = function() {
    ...
    let FormatGraphName = function(graph) {
        ...
        if (graphName.length > MAX_GRAPH_NAME_LENGTH) {
            graphName = graphName.substring(0, MAX_GRAPH_NAME_LENGTH / 2) + "..." + graphName.substring(graphName.length - MAX_GRAPH_NAME_LENGTH / 2);
        }
        return graphName + " " + "<span class=\"bi bi-clock\"></span>" + graph.date.toLocaleDateString(g_language + "-" + g_language);
    };
    for (let i = 0; i < this.lastUsedGraphs.length; i++) {
        let graph = this.lastUsedGraphs[i];
        graphsList.innerHTML = graphsList.innerHTML + "<li>" + "<a class=\"dropdown-item btn btn-sm\" href=\"./?graph=" + graph.graph + "\" role=\"button\">" + FormatGraphName(graph) + "</a>" + "</li>";
    }
}
Application.prototype.PushLastUsedGraph = function(graphName) {
    ...
    let index = this.lastUsedGraphs.findIndex(function(item) {
        return item.graph == graphName;
    });
    if (index >= 0) {
        this.UpdateLastUsedGraphsMenu();
        return;
...
Editor.prototype.initKeyActions = function() {
    ...
    function getChar(event) {
        return String.fromCharCode(getCharCode(event));
    }
...
Editor.prototype.createAlgorithmMenu = function() {
    ...
    for (var i = 0; i < algorithms.length; i++) {
        ...
        button.onclick = function(e) {
            ...
            userAction(this.id);
            self.restButtons("");
            self.application.SetHandler(new AlgorithmGraphHandler(self.application,g_Algorithms[g_AlgorithmIds.indexOf(this.id)](self.application.graph, self.application)));
        }
        var eventData = {};
        eventData.index = i;
        ...
        $("#openAlgorithmList").bind('click', eventData, function(_eventData) {
            ...
            if ((self.application.graph.isMulti() && !algorithm.IsSupportMultiGraph()) || (self.application.graph.hasNegative() && !algorithm.IsSupportNegativeWeight())) {
                $(data.object).hide();
            } else {
                $(data.object).show();
...
$(document).ready(function() {
    ...
    if (!isMobile && !DisableEmscripten) {
        ...
        doIncludeAsync([include(fullPathToGraphffoline), ], () => {
